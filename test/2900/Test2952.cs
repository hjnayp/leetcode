...
public class Test2952
{
    ...
    {
        Solution s = new();
        int[] coins = ArrayParser.ParseOneDimensionalArray<int>("[1,4,10]");
        int target = 19;
        Assert.AreEqual(2, s.MinimumAddedCoins(coins, target));

        coins = ArrayParser.ParseOneDimensionalArray<int>("[1,4,10,5,7,19]");
        target = 19;
        Assert.AreEqual(1, s.MinimumAddedCoins(coins, target));

        coins = ArrayParser.ParseOneDimensionalArray<int>("[1,1,1]");
        target = 20;
